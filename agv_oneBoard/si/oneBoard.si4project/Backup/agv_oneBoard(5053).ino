...
static int ckr_motor_go(void)
{
	int rfid_req = 0, result = 0, line_read_val = 0;
	curr_time = millis();
	...
	if(rfid_check_flag == TRUE)
	{
		...
		if(rfid_req != 0)
		{
			result = rfid_req;
#ifdef DEBUG_MODE
			Serial.print("RFID_REQ: ");
			Serial.println(result);
#endif /* DEBUG_MODE */
		}
	}
#ifdef MOTOR_DEBUG
	Serial.println("motor_go_btn_flag , move_start_by_rfid_tag -> ");
	Serial.println(agv_cb.motor_go_btn_flag);
	...
	{
	#ifdef PARKDY
		ckr_bottom_line_sensor_checker();
	#else
		// 1. compare motor table only ROI -> OK -> according to table
		// 2. compare motor table only ROI -> NG -> search best value in table
		line_read_val = ckr_read_bottom_line();
	
		if(agv_cb.drv_info.turn_direction)	// ckr_jhshin_220714[TURN TABLE] 
		{
			ckr_L_motor_table_set(Mot_Descript_Turn[agv_cb.drv_info.turn_direction][0]);
			ckr_R_motor_table_set(Mot_Descript_Turn[agv_cb.drv_info.turn_direction][1]);
			agv_cb.drv_info.turn_direction = 0;
			//delay(1000);
			Serial.print("Unkown RFID P Time: ");
			Serial.println(curr_time - pre_time);
			pre_time = curr_time;
			return;
		}
		if(agv_cb.drv_info.speed_mode == LOW_SPEED)	// Normal speed
		{
			if(agv_cb.drv_info.drv_direction_info == DIR_LEFTY)
			{
				for(int i = 0; i<21; i++)
				{
					if((line_read_val && Mot_Descript_Lefty[DIR_SPEED][i]) == Mot_Descript_Lefty[DIR_SPEED][i])
					{
						break;
					}
				}
				...
				ckr_L_motor_table_set(Mot_Descript_Lefty[DIR_LEFTY][DIR_LEFTY]);
				ckr_R_motor_table_set(Mot_Descript_Lefty[DIR_RIGHTY][DIR_RIGHTY]);
				//delay(1000);
			}
			else
			{
				ckr_L_motor_table_set(Mot_Descript_Righty[DIR_LEFTY][DIR_LEFTY]);
				ckr_R_motor_table_set(Mot_Descript_Righty[DIR_RIGHTY][DIR_RIGHTY]);
				//delay(1000);
			}
		}
		else
		{
			if(agv_cb.drv_info.drv_direction_info == DIR_LEFTY)
			{
				ckr_L_motor_table_set(HIGH_Mot_Descript_Lefty[DIR_LEFTY][DIR_LEFTY]);
				ckr_R_motor_table_set(HIGH_Mot_Descript_Lefty[DIR_RIGHTY][DIR_RIGHTY]);
				//delay(1000);
			}
			else
			{
				ckr_L_motor_table_set(HIGH_Mot_Descript_Righty[DIR_LEFTY][DIR_LEFTY]);
				ckr_R_motor_table_set(HIGH_Mot_Descript_Righty[DIR_RIGHTY][DIR_RIGHTY]);
				//delay(1000);
			}			
		}
		//if(agv_cb.drv_info.drv_direction_info == DIR_LEFTY)
		...
		ckr_LR_motor_description_set();
		ckr_R_motor_table_set(255);
		ckr_L_motor_table_set(255);
#endif
