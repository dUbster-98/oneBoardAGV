...
static void ckr_linesen_motor_des_init();
...
static int ckr_idle(void)
{
	...
	if(rfid_check_flag == TRUE)
	{
		...
		if(rfid_req != 0)
		{
			Serial.print((String) "IDLE - RFID CHECK : " + rfid_req);
	...
	if (ckr_start_btn_check() || agv_cb.drv_info.move_start_by_rfid_tag == TRUE)
	{
...
static void lcd_display_status_set_value(int status)
{
	if(status != lcd_data.uiTranslateData[LCD_ADDR_DRV_STS])
	{
		...
		Serial.println((String) "be -> af : " + status, lcd_data.uiTranslateData[LCD_ADDR_DRV_STS]);
#endif /* DEBUG_LCD_DP */
...
static int ckr_get_pin_sts(void)
{
	...
	{
		Serial.print((String) "ckr_get_pin_sts : " + PIN_UP);
		result = PIN_UP;
	}
	else
	{
	
		Serial.print((String) "ckr_get_pin_sts : " + PIN_DOWN);
		result = PIN_DOWN;
	}
	return result;
}
...
//
//*********************************************************************************************
static void ckr_move_motor(int line_read_val)
{
	bool find_idx = FALSE;
	int i = 0, j = 0, k = 0, tbl_tmp = 0;
...
void leftyDrive(int line_read_val)
{
	switch(line_read_val)
	{
		...
		case 0b111111111111:	//0b000000000000
			...
			break;
		case 0b111111111110:	//0b100000000000
			Serial.println("># LEFTY 2");
			leftFlag++;
			leftFastMotor();
			break;
		case 0b111111111100:	// 0b110000000000
		case 0b111111111001:	// 0b011000000000
			Serial.println("># LEFTY 1");
			leftFlag++;
			leftMotor();
			break;
		case 0b111111110011:	// 0b001100000000
		case 0b111111100111:	// 0b000110000000
		case 0b111111001111:	// 0b000011000000
		case 0b111110011111:	// 0b000001100000
			Serial.println("># gogo");
			leftFlag=0;
			rightFlag=0;
			goMotor();
		...
		case 0b111100111111:	// 0b000000110000
		case 0b111001111111:	// 0b000000011000
			Serial.println("># righty 1");
			rightFlag++;
			rightMotor();
			break;
		case 0b110011111111:	// 0b000000001100
		case 0b100111111111:	// 0b000000000110
			Serial.println("># righty 2");
			rightFlag++;
			rightFastMotor();
			break;	  

		case 0b001111111111:	// 0b000000000011
		case 0b011111111111:	// 0b000000000001
			Serial.println("># righty 2");
			rightFlag++;
			rightFastMotor();
			break;
...
static void ckr_init()
{
	...
	ckr_linesen_motor_des_init();	// ckr_jhshin_220704[motor_description]
...
//*********************************************************************************************
// Function : ckr_linesen_motor_des_init
//
// Date : 2022-07-04
//
// Author : jhshin 
