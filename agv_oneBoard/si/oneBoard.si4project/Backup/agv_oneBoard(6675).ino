...
void serialEvent2() {
	...
	if(CH2_RXBUFF == 0xBB)  // RFID Buffer 값이 0xBB일때
	{
		...
		if((rfid_buf[0] == 0xAA) && (rfid_buf[2] == 0x00) && (agv_cb.rfid_tag != rfid_buf[1])) 
		{
			...
			lcd_data.uiTranslateData[2] = agv_cb.rfid_tag;
...
static bool ckr_pin_btn_check(void);
...
static int ckr_agv_init(void)
{
	...
	agv_cb.drv_info.direction_info = DIR_LEFTY;
...
static int ckr_agv_stop(void)
{
	...
	if(rfid_check_flag == TRUE)
	{
		rfid_req = ckr_rfid_request_checker(LCD_STOP);
		if(rfid_req != 0)
		{
			agv_cb.request.currReq = rfid_req;
		}
	...
	{
		if(agv_cb.motor_go_btn_flag == FALSE)	// BTN STOP
		{
			agv_cb.request.currReq = AGV_REQ_NONE;
		}
		else							// TAG STOP
...
static int ckr_agv_pin(void)
{
	...
	if(agv_cb.drv_info.pin_action)	// UP and go
	{
		ckr_pin_up_move();
		check_result = ckr_pin_check_stop(PIN_LOW_HIGH);
		Serial.println("ELSE ckr_pin_down_move");
	}
	else	// Down and go
	{
		ckr_pin_down_move();
		check_result = ckr_pin_check_stop(PIN_HIGH_LOW);
...
/*
	...
	// PIN BTN PUSH EVENT PROCESS
	if((_RD_PHOTO_TOP == 1) && (_RD_PHOTO_BOT == 1)) // PIN IS in MIDDLE SIDE
	{
		ckr_pin_down_move();
		Serial.println("ckr_pin_up_move");
		check_result = ckr_pin_check_stop(PIN_HIGH_HIGH);
	}
...
{
	// PIN UP CHECK
	check_result = check_photo_sen_status(RFID_TAG_PINUP_GO);
	if(check_result == R_NG)
	{
...
{
	// PIN DOWN CHECK
	check_result = check_photo_sen_status(RFID_TAG_PINDN_GO);
	if(check_result == R_NG)
	{
		return (AGV_ERROR_STATE);
	}
...
static int ckr_agv_go(void)
{
	...
	ckr_motor_go(agv_cb.drv_info.direction_info);	
...
static void ckr_motor_go(bool direction_info)
{
	...
	// Unknown RFID Input 
	if((agv_cb.motor_go_btn_flag == 1) && (!rfid_req))
	{
	...
	else if((agv_cb.motor_go_btn_flag == 1) && (rfid_req) && (agv_cb.drv_info.move_start_flag))
	{
		ckr_LR_motor_description_set();
		/*
		if((agv_cb.drv_info.speed_mode) && (agv_cb.drv_info.direction_info))	// High speed , righty table select
		{
			HIGH_Mot_Descript_Righty[agv_cb.drv_info.direction_info][BOTTOM_SENSOR_VALUE];
		}
		else if((agv_cb.drv_info.speed_mode) && (!agv_cb.drv_info.direction_info))	// High speed , lefty table select
		{
			HIGH_Mot_Descript_Lefty[agv_cb.drv_info.direction_info][BOTTOM_SENSOR_VALUE];
		}
		else if((!agv_cb.drv_info.speed_mode) && (agv_cb.drv_info.direction_info))	// Normal speed , righty table select
		{
			Mot_Descript_Righty[agv_cb.drv_info.direction_info][BOTTOM_SENSOR_VALUE]; // 가감속 테이블 선택..
		}
		else	// Normal speed , lefty
		{
			Mot_Descript_Lefty[agv_cb.drv_info.direction_info][BOTTOM_SENSOR_VALUE]; // 가감속 테이블 선택..
		}
		*/
...
static void ckr_LR_motor_description_set()
{
...
static int ckr_rfid_request_checker(int curr_function)
{
	...
	if(curr_function == LCD_READY)	// ckr_jhshin_220713[Calling from IDLE function]
	{
		...
		{ 
			case RFID_TAG_STOP:	// ckr_jhshin_220713[just wait in IDLE function] 
				agv_cb.drv_info.move_start_flag = FALSE;
				req_check = AGV_REQ_NONE;
				break;
			case RFID_TAG_PINDN_GO:
				agv_cb.drv_info.pin_action = DOWN_AND_GO;
				req_check = AGV_REQ_STOP;		// ckr_jhshin_220713[Move to STOP -> PIN] 
				break;
			case RFID_TAG_PINUP_GO:
				agv_cb.drv_info.pin_action = UP_AND_GO;
				req_check = AGV_REQ_STOP;		// ckr_jhshin_220713[Move to STOP -> PIN] 
				break;
			...
			case RFID_TAG_L_TURN :
				agv_cb.left_turn = TRUE;
				req_check = AGV_REQ_MOVE;
				break;
			case RFID_TAG_R_TURN:
				agv_cb.left_turn = FALSE;
				req_check = AGV_REQ_MOVE;
				break;			
			case RFID_TAG_START:
				agv_cb.drv_info.move_start_flag = TRUE;
				req_check = AGV_REQ_MOVE;
				break;
			default:
				...
				{
					agv_cb.drv_info.direction_info = DIR_RIGHTY;
					req_check = AGV_REQ_MOVE;
				}
				break;
		}
	...
	else if(curr_function == LCD_STOP)	// ckr_jhshin_220713[Calling from AGV_STOP function]
	{
		Serial.print("ckr_rfid_request_checker : AGV_STOP :");
		Serial.println(agv_cb.rfid_tag);
