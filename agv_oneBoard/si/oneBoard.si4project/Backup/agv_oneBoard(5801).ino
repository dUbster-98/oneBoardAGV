...
//	if ((agv_cb.start_btn_flag == FALSE) && ckr_start_btn_check())
	if ((agv_cb.motor_go_flag == FALSE) && ckr_start_btn_check())
	{
	...
	agv_cb.status.currStatus = rfid_req;
	return (AGV_IDLE_STATE);
}
//*********************************************************************************************
...
static bool ckr_start_btn_check(void)
{
	...
	{
		//agv_cb.start_btn_flag = TRUE;
		agv_cb.motor_go_flag = TRUE;
#ifdef DEBUG_MODE
	...
	else
	{
		...
		agv_cb.motor_go_flag = FALSE;
#ifdef DEBUG_MODE
		Serial.println("START_BUTTON Not Pressed");
#endif /* DEBUG_MODE */
	}
	//return agv_cb.start_btn_flag;
	return agv_cb.motor_go_flag;
}
//*********************************************************************************************
// Function : 
//
...
static int ckr_agv_stop(void)
{
	...
	if(agv_cb.motor_go_flag == 0)	// BTN STOP
	{
		agv_cb.request.currReq = AGV_REQ_NONE;
	}
	else							// TAG STOP
...
static void ckr_motor_go(bool direction_info)
{
...
#else
	if((agv_cb.motor_go_flag == 1) && (!rfid_req))
	{
	...
	else if((agv_cb.motor_go_flag == 1) && (rfid_req))
	{
		ckr_LR_motor_description_set();
		analogWrite(RIGHT_MOTOR, 255);
		analogWrite(LEFT_MOTOR, 255);
...
static int ckr_rfid_request_checker(void)
{
	...
	Serial.print("ckr_rfid_request_checker : ");
	Serial.println(agv_cb.rfid_tag);
...
static void stop_btn_callback(void)
{
	...
	if(agv_cb.motor_go_flag == 1)
	{
		agv_cb.motor_go_flag = 0;
	}
}
...
{
	bool result = FALSE;	
	int photo_sen_top = 0, photo_sen_bot = 0;
	Serial.println("ckr_pin_btn_check");
	curr_time = millis();

	do
	{
		photo_sen_top = digitalRead(PHOTOSEN_TOP);
		photo_sen_bot = digitalRead(PHOTOSEN_BOTTOM);
		//Serial.println(photo_sen_top);
		//Serial.println(photo_sen_bot);
						
		if(((photo_sen_top == 1) && (photo_sen_bot = 0)) || ((photo_sen_top == 00) && (photo_sen_bot = 1)))
		{
			// sensor stop
