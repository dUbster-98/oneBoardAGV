...
unsigned long curr_time = millis();  // ckr_jhshin_220811[Timer Init]  ;
...
void serialEvent2() {
  ...
  //Serial.println((String)"cnt : " + cnt + " -> RF ISR : " + RFID_RXBUFF);
  ...
  if (RFID_RXBUFF == 0xBB)  // RFID Buffer 값이 0xBB일때
  {
    ...
    if ((rfid_buf[0] == 0xAA) && (rfid_buf[2] == 0x00) && (agv_cb.rfid_tag != rfid_buf[1])) {
      ...
      for (int i = 0; i < 8; i++) {
        //Serial.println((String)"># i : "+ i + "val: " + rfid_buf[i]);
        rfid_buf[i] = 0;
      }
...
static int ckr_idle(void) {
  ...
  if (agv_cb.drv_info.move_start_by_rfid_tag == TRUE || ckr_start_btn_check()) {
    Serial.println("># ST btn, RFID Start !!");
...
static bool ckr_start_btn_check(void) {
  ...
  if (digitalRead(START_BUTTON) == HIGH) {
    ...
    //Serial.println("># STS BTN Not Pressed");
  }
  return start_btn;
}
//*********************************************************************************************
...
static void lcd_display_set_init_value() {
...
#ifdef DEBUG_LCD_DP
  Serial.println("#L : lcd_set_init_value");
#endif /* DEBUG_LCD_DP */
  // LCD Display Setting Value Init
  lcd_data.uiTranslateData[LCD_ADDR_E_STOP] = E_STOP_DEFAULT;
  ...
  lcd_data.uiTranslateData[LCD_ADDR_SPEED_ADJ] = agv_cb.mot_speed_adj;   // ckr_khkim3_220826[MOT SPEED ADJ]
  lcd_data.uiTranslateData[LCD_ADDR_TRIGGER] = 0;                        // ckr_jhshin_220701[TRIGGER]
  lcd_data.uiTranslateData[LCD_ADDR_BAT] = analogRead(A4);               // ckr_jhshin_220811[Battery Voltage Check]
}
//*********************************************************************************************
...
#ifdef DEBUG_LCD_DP
    Serial.println("#L: lcd_sts_set_value");
...
//*********************************************************************************************
static void ckr_pin_down_move(void) {
  Serial.println("#P: pin_down_move");
  Serial.println((String) "STS: " + lcd_data.uiTranslateData[LCD_ADDR_PIN_STS]);
  lcd_data.uiTranslateData[LCD_ADDR_PIN_STS] = PIN_DOWN;
  // 경훈씨..LCD에서 전압체크 자동으로 처리하기 힘든 경우, 여기서 정보 올려준다
  lcd_data.uiTranslateData[LCD_ADDR_BAT] = _READ_BATTERY;  // ckr_jhshin_220811[Battery Voltage Check]
  if (_READ_BATTERY < LOW_BAT_ERR) {
    agv_cb.drv_info.move_start_by_rfid_tag = FALSE;
    Serial.println("#BAT: BAT LOW - STOP");
  }

#ifdef DEBUG_LCD_DP
  Serial.println("#L: lcd -> pin_down_move");
#endif /* DEBUG_LCD_DP */
...
static int ckr_pin_check_stop(int status) {
  ...
  unsigned long pin_st_time = millis();
  unsigned long time_check = 0, check_result = 0;
  ...
  do {
    ...
    if ((millis() - pin_st_time) < duration) {
      time_out = 1;
    } else {
#ifdef DEBUG_PIN
  ...
  time_check = millis();
//	Serial.println((String)"st time : " + time_check);
#ifdef NEVER
  do {
    if ((millis() - time_check) > 1000) {
      break;
    }
  } while ((millis() - time_check) < 10000);
#endif /* NEVER */

  check_result = millis() - time_check;
  Serial.println((String) "end time : " + check_result);

  Serial.println("#P: PIN Check END");

#ifdef DEBUG_PIN
  //Serial.println((String) "#P: ckeck_st_time: " + (millis() - pin_st_time));
#endif /* DEBUG_PIN */
...
static void ckr_pin_stop(void) {
  ...
  do {
    if ((digitalRead(PHOTOSEN_BOTTOM) == HIGH) && (digitalRead(PHOTOSEN_TOP) == HIGH)) {
      digitalWrite(PIN_MOTOR_DIR, LOW);
      digitalWrite(PIN_MOTOR_ENABLE, LOW);
      analogWrite(PIN_MOTOR_PWM, 255);
      Serial.println("#M: pin gogo");
    } else {
      pinset = 0;
    }
...
static int ckr_agv_go(void) {
  ...
  //Serial.println((String) "># agv_request" + req_response);
  return (AGV_IDLE_STATE);
}
//*********************************************************************************************
// Function : time_delay_function
//
// Date : 2022-08-30
//
// Author : jhshin
...
//
// Input Parameters
// delay time, time_out value
//
// Output Parameters
// none
//
//*********************************************************************************************
static void time_delay_function(int delay, int time_out) {
  unsigned long cur_time = millis();
  do {
    if ((millis() - cur_time) > delay) {
      break;
    }
  } while ((millis() - cur_time) < time_out);

  Serial.println((String) "# time delay: " + delay);
}
...
void rightDrive(int line_val) {
  ...
  switch (line_val) {
    ...
    default:
      ...
      if (down_side > 1)  //(leftFlag > rightFlag)
      {
#ifdef DEBUG_MOT
...
*/
    ...
    default:
      ...
      if (up_side > 1g)  //(leftFlag > rightFlag)
      {
#ifdef DEBUG_MOT
        Serial.println("#M: --lefty 1");
#endif /* DEBUG_MOT */
...
#ifdef DIR_ORG
  analogWrite(LEFT_MOTOR, 70);
  analogWrite(RIGHT_MOTOR, 20);

  agv_cb.drv_info.speed_mode = 0;
#else
  analogWrite(LEFT_MOTOR, 20);
  analogWrite(RIGHT_MOTOR, 70);
  agv_cb.drv_info.speed_mode = 0;
...
static int ckr_eeprom_read(void) {
  int result = 0;
  int top_num = 0, lefty_num = 0, righty_num = 0, speed = 0;  // char ->int 230207 pdy
...
#else
  top_num = _EEPROM_RD_TOP_RFID;
  lefty_num = _EEPROM_RD_LEFTY;
  righty_num = _EEPROM_RD_RIGHTY;
  speed = _EEPROM_RD_MOT_SPEED;

  agv_cb.top_rfid_number = (int)top_num;
  agv_cb.lefty_tag_number = (int)lefty_num;
  agv_cb.righty_tag_number = (int)righty_num;
  agv_cb.mot_speed_adj = (int)speed;
#endif /* NEVER */
  ...
  if ((agv_cb.top_rfid_number == NULL) || (agv_cb.lefty_tag_number == NULL) || (agv_cb.righty_tag_number == NULL) || (agv_cb.mot_speed_adj == NULL)) {
    Serial.println((String) "># EEPROM NULL");
...
static int ckr_rfid_request_checker(int curr_function) {
  ...
  {
    Serial.println((String) "># rfid_chk:IDLE :" + agv_cb.rfid_tag);
    ...
    switch (agv_cb.rfid_tag) {
      ...
      default:
        ...
        if (agv_cb.rfid_tag == agv_cb.lefty_tag_number) {
          agv_cb.drv_info.drv_direction_info = DIR_LEFTY;
          Serial.println("># Lefty set !!!");
        } else if (agv_cb.rfid_tag == agv_cb.righty_tag_number) {
          agv_cb.drv_info.drv_direction_info = DIR_RIGHTY;
          Serial.println("># Righty set !!!");
        }
        break;
    }
  } else if (curr_function == LCD_MOVE)  // ckr_jhshin_220713[Calling from AGV_GO function, 104: UP, 105: DN]
  {
    Serial.print("># rfid_chk:AGV_GO:");
    ...
    switch (agv_cb.rfid_tag) {
      ...
      default:
        ...
        if (agv_cb.rfid_tag == agv_cb.lefty_tag_number) {
          agv_cb.drv_info.drv_direction_info = DIR_LEFTY;
          Serial.println("># lefty tag input");
        } else if (agv_cb.rfid_tag == agv_cb.righty_tag_number) {
          agv_cb.drv_info.drv_direction_info = DIR_RIGHTY;
          Serial.println("># righty tag input");
        }
        break;
    }
  ...
  Serial.println((String) "># rfid_flag_Rel: " + rfid_check_flag);
  return req_check;
}
...
static bool ckr_pin_btn_check(int rfid_tag) {
  ...
  switch (rfid_tag) {
    case 0:
...
#ifdef DEBUG_PIN
        Serial.println("#P:pin_flotting..HH");
#endif /* DEBUG_PIN */
        ckr_pin_down_move();
      ...
      } else if ((_RD_PHOTO_TOP == 0) && (_RD_PHOTO_BOT == 0)) {
#ifdef DEBUG_PIN
        Serial.println("#P: pin_flotting..LL");
#endif /* DEBUG_PIN */
        ckr_pin_down_move();
        result = ckr_pin_check_stop(PIN_LOW_LOW);
        if (result == FALSE) {
          result = ckr_pin_check_stop(PIN_LOW_LOW);
          if (result == FALSE) {
            lcd_display_status_set_value(E100);
      ...
      {
#ifdef DEBUG_PIN
        Serial.println("#P: pin_down_move..LH");
#endif /* DEBUG_PIN */
        ckr_pin_down_move();
      ...
      {
        Serial.println("#P: pin_up_move..HL");
        ckr_pin_up_move();
...
static void front_sensor_callback(void) {
  ...
  Serial.println((String) "#L :LCD READ: " + lcd_data.uiTranslateData[LCD_ADDR_E_STOP]);

  if (lcd_data.uiTranslateData[LCD_ADDR_E_STOP] == E_STOP_CHANGED) {
    Serial.println((String) "#L :1. LCD RCV DONE: " + lcd_data.uiTranslateData[LCD_ADDR_E_STOP]);
    lcd_data.uiTranslateData[LCD_ADDR_E_STOP] = E_STOP_DEFAULT;
  } else if (lcd_data.uiTranslateData[LCD_ADDR_E_STOP] == E_STOP_DEFAULT) {
    lcd_data.uiTranslateData[LCD_ADDR_E_STOP] = E600;  // ckr_jhshin_220701[DRIVE STS]
    Serial.println((String) "#L :2.LCD SET VAL: " + lcd_data.uiTranslateData[LCD_ADDR_E_STOP]);
    ...
    Serial.println((String) "#L :LCD RCV CKECK: " + lcd_data.uiTranslateData[LCD_ADDR_E_STOP]);
  }

#ifdef NEVER
  if (lcd_data.uiTranslateData[LCD_ADDR_E_STOP] != E600) {
    lcd_data.uiTranslateData[LCD_ADDR_E_STOP] = E600;  // ckr_jhshin_220701[DRIVE STS]
    Serial.println((String) "#L :2.LCD SET VAL: " + lcd_data.uiTranslateData[LCD_ADDR_E_STOP]);
